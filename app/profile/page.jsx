...
export default function ProfilePage() {
  ...
  const handleNameSave = async (firstName, lastName) => {
    ...
    try {
      ...
      await dispatch(getProfile({ id: user.userId, token: user.token }));
  ...
  const handlePhoneSave = async (phone) => {
    if (!user?.userId || !user?.token) return;
    try {
      await dispatch(updateProfile({ id: user.userId, phone, token: user.token })).unwrap();
      await dispatch(getProfile({ id: user.userId, token: user.token }));
